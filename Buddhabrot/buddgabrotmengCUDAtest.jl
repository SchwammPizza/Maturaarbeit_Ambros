...
@time begin
    ...
    vertical2 = floor(Int, zoomPointAsMatrixPoint[2] + m/2)      # zuerst die auf der Komplexe-ebene höchere
    ...
    function berechnungBuddhaBrot!(nzoom::Int64, mzoom::Int64, n::Int64, m::Int64, iteration::Int64, maxValues, mandelbrot, f::CuDeviceVector{ComplexF64, 1}, starty, startx, endy, endx) #schaut was die Maximale Iterationzahl war, und speichert in maxValues wie oft dort ein Punkt ankam
    ...
    function bench_buddhi!(nzoom::Int64, mzoom::Int64, n::Int64, m::Int64, iteration::Int64, maxValues, mandelbrot, starty, endy, startx, endx)
        f = CUDA.zeros(ComplexF64, iteration)
        f .= 3
        numblocks = ceil(Int, length(maxValues)/anzahlThreads)
        CUDA.@sync begin
            @cuda threads=anzahlThreads blocks=numblocks berechnungBuddhaBrot!(nzoom, mzoom, n, m, iteration, maxValues, mandelbrot, f, starty, startx, endy, endx)
        end
    end
    ...
    maxValues = CUDA.zeros(Int128, n, m)
    bench_buddhi!(n*zoom, m*zoom, n, m, iteration, maxValues, mandelbrot, horizontal, horizontal2, vertical, vertical2)
    mandelbrot = nothing
    img = CUDA.zeros(RGB{Float64}, n, m)
    bench_zeich!(n, m, horizontal, vertical, maxValues, img, maximum(img))
    naxValues = nothing
